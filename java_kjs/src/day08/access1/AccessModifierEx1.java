...
public class AccessModifierEx1 {
	...
	public static void main(String[] args) {
		...
		//std.grade = 1; // grade의 접근제어자가 private이기 때문에 같은 패키지에 있는 클래스에서 사용이 불가능
		// grade가 private이어서 grade를 수정하기 위해 setter를 이용하여 수정 
		std.setGrade(1);
		std.classNum = 2; // classNum의 접근제어자가 default여서 같은 패키지에 있는 클래스에서 사용 가능
		std.num = 3; // num의 접근제어자가 public이어서 사용 가능
		System.out.print(std.getGrade() + "학년 ");
		System.out.print(std.getClassNum() + "반 ");
		System.out.println(std.getNum()+"번");
	}
}
