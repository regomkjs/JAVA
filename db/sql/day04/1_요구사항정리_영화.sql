...
- 조회(무비차트)
    ...
    - 영화는 오늘 이후에 일정들을 예매한 내역을 기준으로 조회를 한다.
    - 예매 내역이 많은 순으로 조회한다.
- 조회(상영시간)
    ...
    - 사용자가 영화, 극장, 날짜를 선택하면 극장 정보(극장 이름, 전체 좌석수)와 
	  상영시간 별 현재 예매 가능한 좌석수를 조회할 수 있다.
- 조회(영화 상세 조회)
	- 사용자는 영화 상세 정보를 조회할 수 있다.
    - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일, 트레일러, 스틸컷이 있다.
    - 장르는 등록된 장르를 보여주며, 장르는 여러가지일 수 있다.
    - 기본정보에는 관람가능 연령정보, 런닝 시간(분), 제작국가가 있다.
    - 제작국가는 여러 국가가 될 수 있다.
    - 감독은 여러 감독이 올 수 있다.
...
- 예매
	- 예매는 회원만 가능하다.
	- 상영시간을 선택하면 인원을 선택하고, 좌석을 선택해서 예매할 수 있다.
    - 인원은 성인, 청소년으로 구분한다.
    - 좌석은 선택된 인원수에 맞춰서 예매 가능한 좌석을 선택한다.
    - 예매가 완료되면 예매 번호가 할당되고, 예매 번호를 통해 예매 좌석과 예매 인원 정보를 조회할 수 있다.
    - 예매시 할인은 없다.
...
- 영화 관리
    ...
    - 영화 정보는 영화에 참여한 감독, 배우, 장르, 기본정보, 개봉일, 내용, 스틸컷, 트레일러를 입력하여 등록한다.
    - 감독은 등록된 인물 중 여러명 선택이 가능하다.
    - 배우는 등록된 인물 중 여러명 선택이 가능하다.
...
- 극장 관리
    ...
    - 극장은 지역, 이름, 상영관 정보, 주소, 총 관수를 입력하고 총 좌석수는 등록된 상영관에 맞게 계산이 된다.
...
- 상영관 관리
    ...
    - 좌석 정보 입력이 완료되면 A1부터 좌석 정보가 할당된다.
- 상영시간 관리
	- 관리자는 상영시간 추가/수정/삭제할 수 있다.
	- 상영시간은 상영관정보, 영화, 날짜, 상영시간을 입력하여 등록할 수 있다.
